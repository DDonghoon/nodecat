...
router.get('/guestbooks/update/:id', async (req, res) => {
    try {
        const result = await request(
            req, `/guestbooks/update/`+req.params.id
        );
        console.log(result.data);
        if (result.data.code === 200){

            res.render("guestupdate",{"guestbooks":result.data.data});
        }
...
router.post('/guestbooks/update',async (req, res, next) =>{
    const { "id": id,"name": name, "email" : email, "content": content} = req.body;
    let data = {id, name, email, content }

    try{
        const result = await request_post(req,'/guestbooks/update', data);

        if(result.data.code ===200){
            res.redirect("guestupdate");
        }
...
router.get('/guestbooks/delete/:id', async (req, res) => {

    try {
        const result = await request(
            req, `/guestbooks/delete/`+req.params.id
        );
        console.log(result.data);
        if (result.data.code === 200){
